...
class UserForm(forms.Form):
    ...
    def clean_email(self):
        ...
        if not (email.endswith('vk.team') or email.endswith('corp.mail.ru')):
            raise forms.ValidationError('Используйте корпоротивную почту')
        return email
...
class ManyFieldsFormWidget(forms.Form):
    ...
    is_active = forms.BooleanField(required=False, widget=forms.CheckboxInput(attrs={'class': 'from-check-input'}))
    birthdate = forms.DateField(initial=datetime.date.today,
